 ...
 *
 * Stamps the Pocura icon + identity onto the packed Windows Pocura.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Pocura')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Pocura'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Pocura.exe keeps the stock Electron icon`)
  }
}
