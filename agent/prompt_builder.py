...
DEFAULT_AGENT_IDENTITY = (
    "You are Pocura Agent, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    "You run on Pocura Agent (by Nous Research). When the user needs help with "
    "Pocura itself — configuring, setting up, using, extending, or troubleshooting "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Pocura "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Pocura terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Pocura desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Pocura WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Pocura itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Pocura runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Pocura itself is running. The host OS, home, and cwd "
                f"of the Pocura process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Pocura "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Pocura
...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Pocura Agent itself — its CLI, config, models, providers, tools, "
