# ============================================================================
# Pocura Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HermesHome = $(if ($env:HERMES_HOME) { $env:HERMES_HOME } else { "$env:LOCALAPPDATA\pocura" }),
    [string]$InstallDir = $(if ($env:HERMES_HOME) { "$env:HERMES_HOME\pocura-agent" } else { "$env:LOCALAPPDATA\pocura\pocura-agent" }),
...
$RepoUrlSsh = "git@github.com:ashen437/PocuraAI.git"
$RepoUrlHttps = "https://github.com/ashen437/PocuraAI.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Pocura Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/ashen437/PocuraAI/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/ashen437/PocuraAI/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/ashen437/PocuraAI/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Install-Desktop {
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Pocura.exe",
        "$desktopDir\release\win-arm64-unpacked\Pocura.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Pocura.exe was found under $desktopDir\release\*-unpacked\"
    }
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        # Pre-rebrand shortcuts (Hermes.lnk) are removed rather than renamed
        # in-place: a .lnk that keeps the same path across a rebrand is
        # exactly the case where Windows' shell icon cache sticks with the
        # OLD bitmap even after TargetPath/IconLocation are updated. Deleting
        # and recreating under the new name forces a genuinely fresh icon
        # association. Best-effort: a leftover old shortcut is cosmetic, not
        ...
        foreach ($legacyLnk in $legacyTargets) {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Pocura.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Pocura.lnk')
        )
        ...
        # with the Pocura icon, but without this the shortcut can keep drawing
...
function Write-Completion {
    ...
    Write-Host "$HermesHome\pocura-agent\"
    Write-Host ""
